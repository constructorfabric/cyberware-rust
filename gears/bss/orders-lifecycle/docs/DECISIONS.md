...
| Q-07 | The program retention period for completed and cancelled orders | Product | Append-only with no destructive path; retention deferred to the policy |
...
| Q-13 | D-59 reads PRD §6.2's and §12's "machine-readable **stale-version** reason" as a descriptor and registers `version-conflict`. That reinterprets a `MUST` restated as an acceptance criterion, and was taken autonomously while the adjacent D-58 routed an equivalent PRD-basis question to Product | Product with Architecture | The mapping table in `01 §4.2` stands and is disclosed; confirm the reading, or amend the PRD text so a conformance run against §12 does not fail on a reason name that no longer exists |
