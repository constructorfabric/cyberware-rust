...
* **The gate predicate must exclude the requesting order.** An amendment is issued by an order that already holds its key, so a predicate counting all holders without excluding its own subject refuses every amendment against itself. The transaction avoids the same trap by **partitioning** the resolved keys and re-offering only the ones the order does not already hold — so its own keys are never candidates for conflict. An earlier form avoided it by releasing the prior claims *before* inserting the new ones, which worked for self-collision and introduced a worse defect: the refusal commits under `ADR/0005`, so the release committed with it and a refused amendment gave up its own key. Ordering is now check-then-mutate (`01 §3.6` *Attempt Transition* step 17). This trap is recorded because it has already been introduced twice — by D-83's first form and again by the release-first acquisition.
