...
This plugin is a second, independent realization of the Usage Collector's storage SPI (`UsageCollectorPluginV1`), targeting ClickHouse — a columnar OLAP database — instead of the reference plugin's row-oriented PostgreSQL/TimescaleDB. It is chosen for deployments that weight aggregation-query throughput and storage efficiency over ClickHouse's absence of transactions, row locks, and native foreign keys. The architecture's central engineering problem, and the subject of most of this document, is: **how does a plugin honor the SPI's dedup, deactivation, and referential-integrity contracts on a backend with no ACID transactions and no in-place row update?** The answer is `ReplacingMergeTree(version)` versioned rows for dedup/deactivation — point reads resolve the latest version in SQL (`ORDER BY version DESC LIMIT 1 BY <sort key>`), while the range reads (`list`, `aggregate`) scan raw rows and anti-join the ids that carry a deactivation marker, with duplicate creates prevented at the engine by `insert_deduplication_token` rather than collapsed at read time — and, for referential integrity, **not offering `delete_usage_type` at all**: with no foreign keys and no coordination primitive, a delete could not be ordered against concurrent record inserts, so the catalog is append-only in this backend and the delete-side half of [`plugin-spi.md` Method 9](../../../docs/plugin-spi.md#method-9--delete-usage-type) is returned as `Internal` ("not implemented", [§3.6](#36-interactions--sequences)). A plugin-owned pre-insert catalog check mirrors the structural (not business-logic) role Postgres's native FK plays in the reference plugin on the create side, and is sufficient on its own once types can never disappear. No external lock is used anywhere: every write path is a plain read-then-insert whose concurrency semantics are stated explicitly, per race, in [§3.6](#36-interactions--sequences).
...
| `cpt-cf-usage-collector-adr-0012` (FK-equivalent referential integrity) | Create side: the plugin-owned pre-insert catalog check ([§3.6](#36-interactions--sequences)) rejects a reference to an unregistered type with `UsageTypeNotFound`. Delete side: **not offered** — `delete_usage_type` returns `Internal` ("not implemented") and issues no SQL, so a type can never disappear from under a record and the window `ON DELETE RESTRICT` closes natively in Postgres cannot open here at all. |
...
**Honest degradation over silent narrowing**: every place this backend cannot match the reference plugin's DB-enforced guarantee is either withheld outright (delete-side referential integrity — `delete_usage_type` is not implemented rather than implemented with a race, [§3.6](#36-interactions--sequences)) or implemented as the closest achievable approximation with the residual explicitly documented as a deviation (dedup atomicity is last-writer-wins inside the concurrent window; read-after-write holds per node, not across replicas) — never silently presented as equivalent to what a non-transactional backend cannot actually provide.
...
- **No native `UNIQUE`/`FOREIGN KEY`/`ON CONFLICT`.** Uniqueness (dedup) is emulated at the application level with a best-effort pre-read; referential integrity is emulated at insert time only (catalog existence check) and is complete precisely because usage types are never deleted. ClickHouse's `ReplacingMergeTree` collapsing is the convergence mechanism for duplicate physical rows, not a substitute for either constraint.
...
| Catalog Store (`infra/storage/catalog_store.rs`) | `usage_type_catalog` create / get / list: create (with pre-existence check and idempotency absorb for identical payloads — same `kind`+`metadata_fields` → silent absorb; different `kind`/`metadata_fields` → `UsageTypeAlreadyExists`), get, list (keyset-paginated, fixed `gts_id ASC` order, forward-only cursor). `delete` is not implemented and returns `Internal` without SQL. Spawns a single background catalog-size refresh worker that coalesces mutation-triggered `count()` refreshes via a `tokio::sync::Notify` signal, racing each count against the gear cancellation token for prompt shutdown. |
...
2. **Plugin-owned referential-integrity check** (mirrors the reference plugin's reliance on a DB-native FK firing at insert time — see `cpt-cf-uc-ch-plugin-principle-spi-conformance`, [§2.1](#21-design-principles)): `SELECT gts_id FROM usage_type_catalog WHERE gts_id = ? LIMIT 1` (existence only — invariant across versions, so no resolution is needed). Absent → return `UsageTypeNotFound { gts_id }` (mirrors the reference plugin's FK-violation mapping in `record_store.rs::map_insert_error`). Because usage types are never deleted ([Delete Usage Type](#delete-usage-type--not-implemented-cpt-cf-uc-ch-plugin-seq-delete-type-fk)), a type seen here stays valid for the `INSERT` in step 4; no ordering against a delete is needed, which is how this backend meets [`plugin-spi.md`](../../../docs/plugin-spi.md#method-9--delete-usage-type) Method 9's "MUST NOT admit a window" — by having no delete to race.
...
2. **Concurrent-window semantics (explicit)**: nothing serializes two creates for the same `gts_id`. Both may pass step 1's read before either's `INSERT` is visible, and both then insert; `ReplacingMergeTree(version)` keeps the higher `version` on merge and on read-time resolution, so **the catalog is last-writer-wins inside that window and both callers get `Ok` even when their payloads differ** — `UsageTypeAlreadyExists` is not raised for the loser. Once the winner's row is visible, a later create sees it and returns an absorb or `UsageTypeAlreadyExists`. Since types are never deleted there is no earlier row a re-create would have to outrank, so the version scheme only has to order racing inserts against one another.

#### Delete Usage Type — Not Implemented (`cpt-cf-uc-ch-plugin-seq-delete-type-fk`)

**ID**: `cpt-cf-uc-ch-plugin-seq-delete-type-fk`

`delete_usage_type` is **not implemented** by this backend. The ID is retained so cross-artifact references stay resolvable; the lock-protected verify-then-delete protocol it once named has been withdrawn together with the coordination lock.

1. Log the refusal at `warn` with the `gts_id`.
2. Return `UsageCollectorPluginError::Internal` carrying the fixed message `delete_usage_type is not implemented by the ClickHouse plugin` (`DELETE_UNIMPLEMENTED_MSG`). **No SQL is issued** — an absent `gts_id` is refused identically to a present one (never `UsageTypeNotFound`), and the catalog row is left untouched.

**Why withheld rather than implemented.** ClickHouse has no foreign keys and this plugin has no mutual-exclusion primitive. Without one, a reference-count probe over `usage_records` is only ever a snapshot: a record referencing the type can land between the probe and the `DELETE`, leaving an orphan that `create_usage_record`'s catalog check would then reject on retry while the original row stays behind. `plugin-spi.md` Method 9 forbids admitting that window, and this backend cannot close it — so it does not offer the operation. At the gear boundary the `Internal` variant surfaces as HTTP 500. Removing a mis-registered type is an operator procedure (`DELETE FROM usage_type_catalog WHERE gts_id = '…'`, a lightweight delete on `ReplacingMergeTree`) to be run only while no records reference the type and no ingest for it is in flight. Consequences elsewhere: the `uc_clickhouse_usage_type_referenced_total` counter is not registered (nothing could increment it), and the catalog-size gauge is monotone non-decreasing.
...
**Sorting key (`ORDER BY`)**: `(gts_id)`. **Engine**: `ReplacingMergeTree(version)`. There is no native `PRIMARY KEY`/`UNIQUE` constraint; uniqueness-on-`gts_id` is an application-level invariant enforced best-effort by the create sequence's pre-existence check ([§3.6](#36-interactions--sequences)) and made eventual by version resolution, not a schema-level guarantee. The plugin never deletes rows from this table (`delete_usage_type` is not implemented, [§3.6](#36-interactions--sequences)); no tombstone-flag column or higher-version marker row exists.
